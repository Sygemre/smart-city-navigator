...
import { fetchNearbyVenues, type VenueCategory } from "../services/placesApi";
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <HorizontalScroll horizontal showsHorizontalScrollIndicator={false}>
            {districts.map((item) => (
              ...
              >
                <DistrictLabel isActive={item === selectedDistrict}>{item}</DistrictLabel>
              </DistrictChip>
            ))}
          </HorizontalScroll>
          ...
          {routeWarning ? <WarningText>{routeWarning}</WarningText> : null}

          {route ? (
            <ResultBox>
              <ResultTitle>
                {route.stops.length} durak - {Math.round(route.totalDistanceMeters / 1000)} km
              </ResultTitle>
              {route.stops.map((stop) => (
                <ResultCard key={stop.venue.id}>
                  <VenuePhoto
                    source={{
                      uri: stop.venue.photoUrl ?? "https://images.unsplash.com/photo-1524230572899-a752b3835840?auto=format&fit=crop&w=600&q=70"
                    }}
                  />
                  <CardBody>
                    <StepBadge>Adım {stop.order}</StepBadge>
                    <ResultItemTitle>{stop.venue.name}</ResultItemTitle>
                    <ResultItemMeta>
                      {stop.venue.district} - {stop.visitMinutes} dk - ⭐ {stop.venue.rating.toFixed(1)}
                    </ResultItemMeta>
                    <RouteMeta>
                      Yuru {stop.travelMinutes} dk ({Math.round(stop.travelDistanceMeters)} m) - Baslangic +{stop.startMinuteOffset} dk
                    </RouteMeta>
                    <DirectionsButton onPress={() => openDirections(stop.venue.name, stop.venue.lat, stop.venue.lng)}>
...
const ResultBox = styled.View` margin: 12px 24px 0; padding: 12px; border-radius: 10px; border-width: 1px; border-color: #2A2F3E; background-color: #1C2130; `;
const ResultTitle = styled.Text` color: #FFFFFF; font-size: 14px; font-weight: 700; margin-bottom: 8px; `;
const ResultCard = styled.View` border-radius: 10px; border-width: 1px; border-color: #2A2F3E; overflow: hidden; margin-bottom: 10px; background-color: #1C2130; `;
const VenuePhoto = styled.Image` width: 100%; height: 120px; `;
const CardBody = styled.View` padding: 10px 12px; `;
const StepBadge = styled.Text` align-self: flex-start; padding: 3px 8px; border-radius: 999px; background-color: rgba(255, 92, 53, 0.15); color: #FF5C35; font-size: 11px; font-weight: 700; margin-bottom: 6px; `;
const ResultItemTitle = styled.Text` color: #FFFFFF; font-size: 14px; font-weight: 700; `;
const ResultItemMeta = styled.Text` color: #A0A0A0; font-size: 12px; margin-top: 4px; `;
const RouteMeta = styled.Text` color: #8CC8FF; font-size: 12px; margin-top: 6px; `;
const DirectionsButton = styled.TouchableOpacity` margin-top: 10px; align-self: flex-start; padding: 8px 12px; border-radius: 10px; background-color: rgba(30, 207, 176, 0.18); border-width: 1px; border-color: rgba(30, 207, 176, 0.45); `;
const DirectionsButtonLabel = styled.Text` color: #1ECFB0; font-size: 12px; font-weight: 700; `;